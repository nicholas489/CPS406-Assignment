<template>
    <div class="flex flex-col gap-4 payment-form">
      ...
      <div class="flex items-center gap-2">
        ...
        <input type="text" id="owner-amount" v-model="ownerPaymentAmount" @input="validateAmount" class="w-32 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Amount" />
      </div>
...
const ownerTotalAmountDue = ref(5000); // Use ref to make it reactive
...
function submitOwnerPayment() {
  ...
  if (ownerPaymentAmount.value === '') {
    ...
    // Subtract the payment amount from the total amount due
    ownerTotalAmountDue.value -= payment;
    toast.add({
      severity: 'success',
